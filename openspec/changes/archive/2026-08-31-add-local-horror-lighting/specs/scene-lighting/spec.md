...
**Reason**: Global directional illumination and visibility-preserving ambient fill conflict with the local pools of light and surrounding darkness required by the ambient-horror direction.

**Migration**: Use the two immutable point lights and near-black ambient contribution defined by `Immutable prototype environment light` and `Bounded local diffuse scene shading`.
