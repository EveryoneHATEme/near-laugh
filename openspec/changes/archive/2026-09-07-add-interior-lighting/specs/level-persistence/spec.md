...
- **THEN** the normalized document starts clean and the editor identifies that an explicit save writes version 8, which older builds cannot read
...
- **THEN** all door IDs, order, configurations, entry/default/switch state, legacy chair collision and appearance are preserved in the normalized version-8 document without writing the source
...
- **THEN** its authored content is preserved with empty audio collections, the source is unchanged, and the editor opens it clean with notice that explicit saving writes version 8
...
- **THEN** v8 normalization preserves parameters/order/audio and produces point-light-0 initially on, point-light-1 initially off, both unshadowed, and light-switch-0 referencing point-light-1 without writing the source

#### Scenario: Current light arrays use the wrong shape
- **WHEN** v8 omits light_switches, contains light_switch, omits required light IDs/flags, or puts a light index or initial state on a switch
...
- **THEN** output uses version 8 with an empty switch array, empty doors, and the `default` entry while preserving original authored content

#### Scenario: Version-3 document is explicitly saved
- **WHEN** the user saves a normalized version-3 document
- **THEN** output uses version 8 with empty doors and preserves the switch and other authored values including the original spawn as `default`

#### Scenario: Version-4 document is explicitly saved
- **WHEN** the user saves a normalized version-4 document without other edits
- **THEN** output uses version 8 with empty doors and unchanged entries, default, terrain presence, lights, normalized chair placement/materials, and switch

#### Scenario: Version-5 document is explicitly saved
- **WHEN** the author saves a normalized version-5 level
- **THEN** version-8 output retains every door field and order alongside the mapped chair and structural materials, and reloading/saving is byte-identical
...
- **THEN** output uses version 8 with empty audio collections and unchanged prior content

#### Scenario: Light identity survives collection edits
- **WHEN** a valid v8 document with shared switch links and reordered lights is saved, loaded and saved again
...
- **THEN** output is canonical v8 with the deterministic light/switch migration and unchanged audio/door/prop content
