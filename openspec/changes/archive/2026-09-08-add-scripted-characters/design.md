...
footsteps use ambience-kind cues of at most 0.15 seconds. The neutral interaction
uses a one-second essential cue with at most 0.25 seconds of audible effect
followed by silence and a matching one-second Russian caption. The full
one-second cue governs foreground arbitration and logical completion, preserving
the existing minimum caption duration.
...
Authored marks and published feet remain ground-surface anchors for route
arrival, rendering and audio. On slopes, derive a separate vertical capsule
offset from accepted support so the upright capsule clears the surface. Keep
the catalog dimensions and existing slope/arrival limits. Shared initial
validation uses the same offset for static, overhead, entry, actor and door
clearance. Sweeps cover actual capsule placement and changes in the offset;
changing support never permits an unchecked lift or snap through a blocker.
Flat ground has zero offset. No inverse kinematics is introduced.
