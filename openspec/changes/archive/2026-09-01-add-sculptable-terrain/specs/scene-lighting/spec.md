...
- **WHEN** the built-in axis-aligned solids and terrain grid are converted to opaque triangle vertices
- **THEN** each triangle's vertices contain matching world-space positions and an outward-facing unit normal

#### Scenario: A scene fragment is shaded
- **WHEN** an opaque prototype-solid or terrain triangle produces a covered fragment
...
- **THEN** the scene retains the existing two authored point lights and readable ambient lighting without a spot contribution
