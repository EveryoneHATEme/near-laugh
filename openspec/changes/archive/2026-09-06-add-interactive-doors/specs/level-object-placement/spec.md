...
The editor SHALL present one flat selectable set containing every axis-aligned solid and named entry, exactly two point lights, the single packaged static-prop placement with its box proxy, the optional singleton switch, and every authored hinged door. The editor SHALL allow solids and entries to be added, duplicated, and removed within their bounds and the switch to be added when absent and removed when present. Doors SHALL support add, duplicate, and remove within their thirty-two-door bound. It SHALL NOT create or remove the fixed-count lights or prop placement or introduce unsupported object, asset, component, or hierarchy types. The default entry SHALL be visibly identified.

#### Scenario: Level objects are listed
- **WHEN** a valid level document is active
- **THEN** the object list contains each supported object exactly once with its concrete game-specific type, entry or door identifier where applicable, and no parent-child hierarchy
...
### Requirement: Door authoring and initial-state preview
The editor SHALL let the author add a door, select it from the object list or its visible initial leaf, edit its concrete fields, rename or remove it, and preview the authored open and lock state without running gameplay simulation. Door creation, duplication, deletion, committed property edits, and surface placement SHALL use the existing bounded history and saving/playtest validation. Renderable gameplay-invalid placements SHALL remain editable with door-specific diagnostics. Fields that cannot form finite bounded geometry SHALL be rejected on edit or safely omitted from preview with a diagnostic when opening a structurally safe invalid document. Preview SHALL show the hinge, swing direction and endpoint, and lock side for the selected door without making those overlays part of the runtime.

#### Scenario: Door is added and configured
- **WHEN** the author adds a door below the count bound and edits its hinge, size, angle, initial pose, or lock side
- **THEN** one selected definition with a unique durable identifier appears and each committed operation participates in preview, validation, dirty state, and undo/redo

#### Scenario: Door is placed on an upper floor
- **WHEN** the author selects an upward structural surface for door placement
- **THEN** the bottom hinge is placed above that surface by the displayed clearance offset in one edit, while yaw, dimensions, identity, and states are preserved

#### Scenario: Door intersects an entry
- **WHEN** a finite committed door edit causes its initial leaf to overlap any standing entry
- **THEN** both the door and entry remain inspectable with validation diagnostics and saving and playtesting remain unavailable until correction or undo

#### Scenario: Preview recovers or terrain is sculpted
- **WHEN** a document with doors is resized, restored, sculpted, undone, or redone
- **THEN** preview uses the current authored initial poses and lock appearance without stale runtime motion, lost doors, or a physics dependency
