...
- **THEN** the first rendered player-camera frame shows multiple recognizable 3D surfaces at different distances rather than the previous clip-space triangle
...
- **THEN** the built-in scene and collision remain available without additional external scene or collision assets
