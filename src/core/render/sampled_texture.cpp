...
VkImageSubresourceRange colorRange(std::uint32_t base_mip,
  ...
  return {VK_IMAGE_ASPECT_COLOR_BIT, base_mip, level_count, 0, 1};
}
}  // namespace

SampledTexture::SampledTexture(VkDevice device,
    ...
    : device_(device),
      ...
      width_(material.image.width),
      height_(material.image.height),
      nearest_(material.nearest) {
  ...
  try {
    createImageAndUpload(graphics_queue, graphics_queue_family, material.image);
    createImageView();
    createSampler();
...
void SampledTexture::createImageAndUpload(VkQueue graphics_queue,
                                          std::uint32_t graphics_queue_family,
                                          const DecodedRgbaImage& image) {
  if (width_ == 0 || height_ == 0 || width_ > 2048 || height_ > 2048 ||
      image.pixels.size() != static_cast<std::size_t>(width_) * height_ * 4)
    throw std::runtime_error("Material image dimensions/pixels are invalid");
  const auto& staging_pixels = image.pixels;

  VkFormatProperties format_properties{};
  vkGetPhysicalDeviceFormatProperties(
      physical_device_, prototype_texture_format, &format_properties);
  requirePrototypeTextureFormatFeatures(
      format_properties.optimalTilingFeatures);
  mip_level_count_ = fullMipLevelCount(width_, height_);
  ...
  try {
    ...
    staging_allocation.memoryTypeIndex =
        chooseMemoryType(staging_requirements.memoryTypeBits, memory_properties,
    ...
    void* mapped = nullptr;
    requireVulkan(
        vkMapMemory(device_, staging_memory, 0, staging_byte_count, 0, &mapped),
    ...
    requireVulkan(
        vkAllocateMemory(device_, &image_allocation, nullptr, &image_memory_),
    ...
    requireVulkan(
        vkAllocateCommandBuffers(device_, &command_allocation, &command_buffer),
    ...
    std::int32_t source_width = static_cast<std::int32_t>(width_);
    std::int32_t source_height = static_cast<std::int32_t>(height_);
    for (std::uint32_t mip = 1; mip < mip_level_count_; ++mip) {
      ...
      blit_region.srcSubresource = {VK_IMAGE_ASPECT_COLOR_BIT, mip - 1, 0, 1};
      blit_region.srcOffsets[1] = {source_width, source_height, 1};
      blit_region.dstSubresource = {VK_IMAGE_ASPECT_COLOR_BIT, mip, 0, 1};
    ...
    to_shader_read[1].subresourceRange = colorRange(mip_level_count_ - 1, 1);
    dependency.imageMemoryBarrierCount = mip_level_count_ == 1 ? 1 : 2;
    dependency.pImageMemoryBarriers =
        to_shader_read.data() + (mip_level_count_ == 1 ? 1 : 0);
    vkCmdPipelineBarrier2(command_buffer, &dependency);
    ...
    requireVulkan(vkQueueSubmit2(graphics_queue, 1, &submit_info, upload_fence),
                  "Submit prototype texture upload with Synchronization 2");
    requireVulkan(vkWaitForFences(device_, 1, &upload_fence, VK_TRUE,
...
void SampledTexture::createSampler() {
  ...
  sampler_info.magFilter = nearest_ ? VK_FILTER_NEAREST : VK_FILTER_LINEAR;
  sampler_info.minFilter = nearest_ ? VK_FILTER_NEAREST : VK_FILTER_LINEAR;
  sampler_info.mipmapMode =
      nearest_ ? VK_SAMPLER_MIPMAP_MODE_NEAREST : VK_SAMPLER_MIPMAP_MODE_LINEAR;
...
void SampledTexture::createDescriptor(const SceneMaterialData& material) {
  ...
  } uniform{material.base_color_factor,
            {material.alpha_cutoff, material.alpha_mask ? 1.0F : 0.0F, 0, 0}};
  static_assert(sizeof(MaterialUniform) == 32);
  VkBufferCreateInfo buffer_info{VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO};
  buffer_info.size = sizeof(uniform);
  buffer_info.usage = VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT;
  buffer_info.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
  requireVulkan(
      vkCreateBuffer(device_, &buffer_info, nullptr, &material_buffer_),
      "Create material uniform buffer");
  recordLifecycleEvent("material.buffer.created");
  if (forcedVulkanFailureAt("material_buffer"))
    throw std::runtime_error("Forced material uniform buffer failure");
  VkMemoryRequirements requirements{};
  vkGetBufferMemoryRequirements(device_, material_buffer_, &requirements);
  ...
  memory.memoryTypeIndex =
      chooseMemoryType(requirements.memoryTypeBits, properties,
                       VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                           VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                       "material uniform");
  requireVulkan(vkAllocateMemory(device_, &memory, nullptr, &material_memory_),
                "Allocate material uniform memory");
  recordLifecycleEvent("material.memory.allocated");
  if (forcedVulkanFailureAt("material_memory"))
    throw std::runtime_error("Forced material uniform memory failure");
  requireVulkan(
      vkBindBufferMemory(device_, material_buffer_, material_memory_, 0),
      "Bind material uniform memory");
  void* mapped = nullptr;
  requireVulkan(
      vkMapMemory(device_, material_memory_, 0, sizeof(uniform), 0, &mapped),
      "Map material uniform memory");
  std::memcpy(mapped, &uniform, sizeof(uniform));
  vkUnmapMemory(device_, material_memory_);
  recordLifecycleEvent("material.uploaded");
  if (forcedVulkanFailureAt("material_upload"))
    throw std::runtime_error("Forced material uniform upload failure");
  VkDescriptorBufferInfo buffer{material_buffer_, 0, sizeof(uniform)};
  VkWriteDescriptorSet parameters{VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET};
  parameters.dstSet = descriptor_set_;
  parameters.dstBinding = 1;
  parameters.descriptorCount = 1;
  parameters.descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
  parameters.pBufferInfo = &buffer;
  const std::array<VkWriteDescriptorSet, 2> writes{write, parameters};
...
void SampledTexture::cleanup() noexcept {
  ...
  if (material_buffer_ != VK_NULL_HANDLE) {
    vkDestroyBuffer(device_, material_buffer_, nullptr);
    material_buffer_ = VK_NULL_HANDLE;
    recordLifecycleEvent("material.buffer.destroyed");
  }
  if (material_memory_ != VK_NULL_HANDLE) {
    vkFreeMemory(device_, material_memory_, nullptr);
    material_memory_ = VK_NULL_HANDLE;
    recordLifecycleEvent("material.memory.freed");
  }
